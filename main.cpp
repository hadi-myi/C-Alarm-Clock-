...
int main() {
	...
	// Regular clock time should be unchanged while we've been setting the alarm
	// time.
	ac2.AlarmButton();
	ac2.DisplayTime(); // Expect 03:33
	ac2.DisplayAmPm(); // Expect PM
	...
	std::cout << std::endl
			  << "Testing out a clock with the 3rd constructor." << std::endl;
	...
	for (int i = 0; i < 23; i++) {
		...
		ac3.DisplayTime(); // Clock should loop around every hour back to 11:01
						   // PM
	...
	// Alarm time should sitll be set to 11:59 PM, as that was when the alarm
	// went off initially
	ac3.AlarmButton();
	ac3.DisplayTime(); // Expect 11:59 BEEP BEEP BEEP
	ac3.DisplayAmPm(); // Expect PM BEEP BEEP BEEP
					   /**
						 ...
						 for (int i=0; i<4; i++) {
						   ac3.AdvanceMinute();  // Re-snoozing a snoozed alarm should not change
						 the snooze timer. ac3.DisplayTime(); ac3.DisplayAmPm();
						   ac3.DisplayAlarmSet();  // Expect Alarm is ON
						   ac3.SnoozeButton();
						 }
						 ...
						 ac3.SnoozeButton();  // Snoozing again, to check that it won't sound again
						 even if alarm is turned off. ac3.DisplayTime();   // Expect 12:12
						 ac3.DisplayAmPm();   // Expect AM
						 ac3.DisplayAlarmSet();  // Expect Alarm is ON
